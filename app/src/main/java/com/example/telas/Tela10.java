...
public class Tela10 extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        rbgSera = findViewById(R.id.rbgSera);
    ...
    public void Confirm(View view) {
        int idChecked =rbgSera.getCheckedRadioButtonId();

        int pontos = getIntent().getIntExtra("pontos", 0);
        String user = getIntent().getStringExtra("user");

        if (idChecked == R.id.rbSera) {
            pontos++;
        }
        textTitle.setText(String.valueOf(pontos));


        Intent it = new Intent(this, Tela11.class);
